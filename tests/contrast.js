 ...
 *
 * WCAG AA (4.5:1) check of every text/background token pair in the three
 * themes, read straight from css/app.css. Exits 1 on any failure.
 * (ui-kit.html shows the same table computed live in the browser.)
 */
const fs=require('fs'),path=require('path');const css=fs.readFileSync(path.join(__dirname,'..','css/app.css'),'utf8');
function block(sel){const i=css.indexOf(sel);if(i<0)throw sel;const s=css.indexOf('{',i);let d=0,j=s;for(;j<css.length;j++){if(css[j]=='{')d++;if(css[j]=='}'){d--;if(!d)break}}return css.slice(s+1,j)}
function vars(b){const o={};b.replace(/--([\w-]+):\s*([^;]+);/g,(m,k,v)=>{o[k]=v.trim()});return o}
const T={dark:vars(block(':root,\n[data-theme="dark"]')),light:vars(block('[data-theme="light"] {')),hc:vars(block('[data-theme="hc"] {\n'))};
...
for(const[t,v]of Object.entries(T))for(const[f,b]of pairs){let bg=parse(v[b]);if(bg[3]<1)bg=over(bg,parse(v.surface));let fg=parse(v[f]);if(fg[3]<1)fg=over(fg,bg);const r=ratio(fg,bg);if(r<4.5)bad++;if(r<4.5||all)console.log(t.padEnd(6),(f+' on '+b).padEnd(30),r.toFixed(2),r>=4.5?'AA':'FAIL')}
