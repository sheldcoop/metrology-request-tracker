...
| 22 | 2026-09-24, Prince | **Future: Hirata code on lots** | Engineers usually get a panel as a Hirata code (a dot pattern). Prince gives a small working HTML app (Hirata coder / decoder); it gets built into this app, and the user can decode a Hirata code and **attach the panel number(s) to a lot** from there. Open until the app arrives: what the code holds (lot number? panel number? more?), paste vs camera/scanner input, and whether a lot keeps the decoded code per panel. Must stay offline (`file://`, no CDN) - check the app's dependencies first. |
